...
(function($) {
   ...
   * @type jQuery
   * @example $("#myselect").addOption("Value", "Text"); // add single value
   *          (will be selected)
   * @example $("#myselect").addOption("Value 2", "Text 2", false); // add
   *          single value (won't be selected)
  ...
  /**
   * Removes an option (by value or index) from a select box (or series of
   * select boxes)
   * 
   * @name removeOption
   * @author Sam Collett (http://www.texotela.co.uk)
   ...
   * @example $("#myselect").removeOption(/^val/i); // remove options with a
   *          value starting with 'val'
   * @example $("#myselect").removeOption(/./); // remove all options
   * @example $("#myselect").removeOption(/./, true); // remove all options that
   *          have been selected
   * @example $("#myselect").removeOption(0); // remove by index
   * @example $("#myselect").removeOption(["myselect_1","myselect_2"]); //
   *          values contained in passed array
   * 
   */
  ...
  /**
   * Removes an option by text from a select box (or series of select boxes)
   * 
   * @name removeOptionByText
   * @author Shaun Moss
   * @type jQuery
   * @param String The text of the Option to remove
   * @param Boolean
   *          selectedOnly (optional) Remove only if it has been selected
   *          (default false)
   * @example $("#myselect").removeOption("Text"); // remove by text
   */
  $.fn.removeOptionByText = function() {
    var a = arguments;
    if (a.length != 1) {
      return this;
    }
    var ta = typeof(a[0]);
    var v, index;
    // has to be a number, string or array of strings:
    if (ta == "number" || ta == "string" || ta == "object") {
      v = a[0];
      // if an array, remove items
      if (v.constructor == Array) {
        var l = v.length;
   ...
   *          above
   * @example $("#myselect").copyOptions("#myselect2","all"); // copy all
   *          options from 'myselect' to 'myselect2'
   * 
   */
   ...
   * @example if($("#myselect").containsOption("val1")) alert("Has an option
   *          with the value 'val1'");
   * @example if($("#myselect").containsOption(/^val/i)) alert("Has an option
   *          with the value starting with 'val'");
